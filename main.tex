...
\begin{titlepage}
   \begin{center}
        ...
        \vspace{0.5cm}
        \LARGE{Prediction of RNA 3D structure using gradient-descending and knowledge-based potential}
...
Predicting the tertiary RNA structure depends also on how we represent the molecule. There are different structural representations : 

\begin{itemize}
    \item All-Atom Representation : This repsentation use all atom from the RNA molecule, sometimes even hydrogen atom. This approach provides the most accurate physicochemical description of the molecule. Unfortunately, this representation have also a high computational cost because of the sheer number of degrees of freedom \cite{sponer_rna_2018}, so it's not used for the large RNA molecule. 
    \item Multi-beads Models : This representation use between 3 to 7 atoms to modeling the RNA. Unlike the all-atom representation, these models balanced structural detail and computational speed. Some predicting model have used these model, for example SimRNA \cite{10.1093/nar/gkv1479} used a 3-bead model, one for the sugar ring, a another for the phosphate group and one for the nucleobase. Also, HiRE-RNA \cite{10.1021/jp102497y} and oxRNA \cite{10.1063/1.4881424} used a 5 to 7 beads to differentiate the Watson-Crick interacting edges from the Hoogsteen edges, they have also capture finer topological details. 
    \item Single-beads Models : This representation used only one atom to modeling the all nucleotide. This approach sacrifices explicit local structural nuances but it's smoothing the thermodynamic energy landscape. One prediction model used this representation, NAST \cite{jonikas_coarse-grained_2009}, and it's very efficient for exploring global topological arrangements and simulating large-scale folding pathways.
\end{itemize}

To represents the RNA, we used a single-beads representation, because of it's lower computational cost and for exploring more easily the global topological arrangements. But with this representation, we need to decide which atom, from the nucleotide, we will used. To resolve this problem, we selectionned three atom : 
\begin{itemize}
...
Thanks to the interpolation and the others terms, the objective function can ensure that no physical laws are being violated. So the results can be used to other application. Since a present you the objective function, i will now present you my algorithm. 
...
As mentioned above, the statistical potential interpolation have a lot of local minima, and a gradient descending alone have tendency to be trapped in a local minima. To overcome this problem, we added a global optimization loop (see Fig.~\ref{fig:global-optim}, Appendix).
...
\begin{figure}[h!]
\begin{subfigure}{0.5\textwidth}
\includegraphics[width=0.9\linewidth]{img/original_structure.png} 
\caption{Original Structure}
\label{fig:original-structure}
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
\includegraphics[width=0.9\linewidth]{img/predicted_structure_C1.png}
\caption{Predicted Structure using C1' representation. Score final : 2.3959}
\label{fig:predicted-structure-C1}
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
\includegraphics[width=0.9\linewidth]{img/predicted_structure_C4.png} 
\caption{Predicted Structure using C4' representation. Score final : 0.4232}
\label{fig:predicted-structure-C4}
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
\includegraphics[width=0.9\linewidth]{img/predicted_structure_C3.png} 
\caption{Predicted Structure using C3' representation. Score final : 3.9621}
\label{fig:predicted-structure-C3}
\end{subfigure}
\caption{The original and predicted structures of a transfer RNA from \textit{Azoarcus sp. BH72} (PDB ID : 9X6B) \cite{hong_pre-splicing_2026}. (a) The original structure. (b) the final stucture of the folding using RMSD and C1'. (c) the final stucture of the folding using RMSD and C4'. (d) the final stucture of the folding using RMSD and C3'}
\label{fig:comparaison-structure-folding}
\end{figure}

The predicted structure using metrics instead of a knowledge-based potential seems more precise and more accurate compared to the original structure (see Fig \ref{fig:original-structure}). Even if, all predicted structure seems to be the same, we can distingued, with the C4' representation (see Fig. \ref{fig:predicted-structure-C4}), a helice on RNA like on the original. This observation can be verified also by its final score, who its more lower than the other representation, and with the RMSD more lower it's the number, more the structure are close. 

At first, using a metric instead of a knowledge-based potential was to make a control on the optimization algorithm, to know if the algorithm can work to approch the original structure, and can reduce the energy. As we can see on the figure \ref{fig:score-comparaison-folding}, our algorithm reduce pretty quickly the energy. The C1' representation reduce faster than the others but the C4' representation arrived to have a better energy with roughly the same number of epochs. We can see also that even if we put noise, every 10 000 epochs, the algorithm arrived to minimizing the score. This results is encouraging because this proves that our optimization methods can 

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{img/score_comparison.png}
    \caption{Line chart representing the objective function evolution by epochs for each representation of the transfer RNA from \textit{Azoarcus sp. BH72} (PDB ID : 9X6B) \cite{hong_pre-splicing_2026}}
    \label{fig:score-comparaison-folding}
\end{figure}

\subsection{Internet platform for RNA folding}

With the discovery of making good folding path. We retrieved all RNA with a resolution under 3.0 \r{A} inside the PDB and we used them for making all folding path of each RNA. Thanks to, three other interns, who arrived after me, we build a website \cite{arda}, name ARDA for Archive of RNA Dynamics and Analysis, to show all the database of folding path. On this website, we can see also some information about the algorithm like the representation methods used, the version, or processing time (see Fig. \ref{fig:info-section}). Also, we can see multiples informations about the folding path. For example, we put a animation of each PDB file to make a folding movie. We also put a line chart to see how each atoms perfoms to reduce the score (see Fig. \ref{fig:line-chart}). Finally we put a contact maps (see Fig. \ref{fig:contact-maps}), its a squared matrix that indicates if a nucleotide are in contact. To know if there are in contact, we calculate the distance between atom, if there are inferior of a threshold than there is contact else there is no contact. This graph permit to capturates the tertiary topology, this means each nucleotide are closely in space. In our case, we used the frequency of contact during the construction of the folding path. 

\begin{figure}[h!]
\begin{subfigure}{0.5\textwidth}
\includegraphics[width=0.9\linewidth]{img/info-arda.png} 
\caption{Section about algorithm's information on the ARDA site}
\label{fig:info-section}
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
\includegraphics[width=0.9\linewidth]{img/line-chart-arda.png}
\caption{Line chart section representing the objective function evolution by epochs for each atoms on the ARDA site}
\label{fig:line-chart}
\end{subfigure}
\begin{subfigure}{0.5\textwidth}
\includegraphics[width=0.9\linewidth]{img/contact-maps-arda.png}
\caption{Contact maps section on the ARDA}
\label{fig:contact-maps}
\end{subfigure}

\caption{Screenshots of the ARDA website presenting the folding path of the transfer RNA from \textit{Azoarcus sp. BH72} (PDB ID : 9X6B) \cite{hong_pre-splicing_2026}, Source : \cite{arda-example}}
\label{fig:image2}
\end{figure}
...
To make a benchmark of our optimization algorithm, we used three different test set. The first one is RNA-Puzzles \cite{b56a52a4a212429e973b1fcd72be4f04}. It's a collaborative test set from the community. There is multiple type of RNA such as multi-stranded structures, ribozymes, riboswitches. There is 24 pdb file but with multiple chain inside some of them, there is 35 RNAs with sequence between 10 to 188 nucleotides. The second one is a collaboration between RNA-Puzzles and CASP teams led to the CASP15 \cite{https://doi.org/10.1002/prot.26602} dataset. Its contains 14 RNAs with sequence between 28 to 720 nucleotides. This test-set will tell us if our algorithm can predict long-RNA. The final test set is constructed one from RNAsolo \cite{10.1093/bioinformatics/btac386}. We selected RNA with a resolution below 4 \r{A} and removed the structure identity higher than 80\%. This results with 29 RNA molecule with a sequence between 40 to 298 nucleotides. 

\subsection{Evaluation metrics}

To evaluate and conclude on the performance of our model, we selected 6 different metrics : 
\begin{itemize}
    \item \acrfull{rmsd} : This metric is sensible on the local difference. It superimpose the predicted structure on the reference one and compute the root mean square of the distance between each equivalent atom. The lower the RMSD, the closer the predicted structure is to the reference, with 0 meaning a perfect match. Because the errors are squared before being averaged, this metric is very sensible to outliers, a single badly predicted region can inflate the whole score even if the rest of the structure is well predicted.
    \item \acrfull{mae} : This metric is close to the RMSD but it use the absolute difference instead of the squared one, and average it. This makes it less sensible to outliers than the RMSD, a bad local region will not dominate the score as much. Like the RMSD, a lower value indicates a better prediction, and 0 is a perfect match.
    \item \acrfull{inf} \cite{PMID:19710185}: This metric used RNA key interaction to evaluate more precisely. It evaluate three interaction : the Watson-Crick base pairs, the non-Watson-Crick base pairs and the base stacking. For each type, it compare the interaction found in the predicted structure to the ones of the reference, using a precision/recall type of formula, to know if the key interaction are conserved. The INF is between 0 and 1, the closer to 1, the more the RNA key interaction are correctly predicted.
    \item TM-score \cite{https://doi.org/10.1002/prot.20264, 10.1093/bioinformatics/btz282} : This metric evaluate the global fold similarity between the predicted and the reference structure, independently of the sequence length. It use a scoring function that give less weight to the large distance error, so it is less sensible than the RMSD to a locally misfolded region. The TM-score is between 0 and 1, 1 being a perfect match. A score above 0.45 is generally consider as indicating that the two structures share the same global fold.
    \item lDDT \cite{10.1093/bioinformatics/btt473} : This metric evaluate the local geometry of the structure, without needing a global superposition. For each atom, it look at its neighborhood and check if the distances to the surrounding atoms are conserved between the predicted and the reference structure, within different tolerance thresholds. The lDDT is between 0 and 100, the higher the better. Because it does not rely on a global alignment, it is less sensible to domain movement or flexible region than the RMSD or the TM-score.
    \item GDT-TS \cite{https://doi.org/10.1002/(SICI)1097-0134(1999)37:3+<22::AID-PROT5>3.0.CO;2-W}: This metric, after superimposing the two structures, compute the percentage of atoms that are within a given distance cutoff (usually 1, 2, 4 and 8 \r{A}) from their equivalent in the reference structure, and average these percentages. The GDT-TS is between 0 and 100, the higher the score, the more atoms are well predicted. 
\end{itemize}
...
Despite the predicted part, doesn't work as espected, we find a model that can produce a complete folding path using metrics instead knowledge-based potential. This finding can be used to, theoretically, make a new model that will predicting the final 3D structure of a RNA but also the folding path of it. If this model can work, we would have a new way to predict RNA and have a new fields to comprehend how RNA folds, and maybe also find new drugs and comprehend also some illness. 
...
This appendix gathers supplementary material that supports the main text but is too detailed to include there directly.

\subsection*{Pipeline diagrams}
\label{app:pipeline-diagrams}

The following pages present, in landscape orientation, the diagrams referenced in Section~\ref{sec:materials-methods} illustrating the local optimization phase, the global optimization phase, and the complete pipeline.

\begin{landscape}
\begin{figure}[p]
    \centering
    \includegraphics[width=\linewidth]{diagrams/local_optim.png}
...
\begin{figure}[p]
    \centering
    \includegraphics[width=\linewidth]{diagrams/global_optim.png}
